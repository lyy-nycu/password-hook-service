...
| Trusted proxy CIDRs | **Resolved 2026-07-20:** `10.0.8.0/26` (the staging `agw-subnet` in `vnet-ag-stg-jpe-001`). Confirmed via Microsoft's documented Application Gateway v2 behavior: backend connections always originate from a private IP in the gateway's own subnet, never a single fixed IP, so the entire subnet must be trusted rather than one address ([Application Gateway FAQ](https://learn.microsoft.com/en-us/azure/application-gateway/faq#what-source-ip-addresses-should-i-allow-to-my-backend-servers)). This has not yet been confirmed by an actual end-to-end signed request through the live path (Pass 2/Container App is not yet deployed); re-verify against the application's own resolved-peer logs once Pass 2 is live and before production rollout. | No longer blocking for staging config; empirical confirmation via live traffic remains outstanding before production |
| Container App ingress mode | **Corrected 2026-07-21:** `external_enabled = true` (was originally `false`/"internal-only" per Task 6's checklist). Staging validation proved the original internal-ingress design does not work with the Application Gateway backend: Microsoft's ingress model scopes "internal" ingress to app-to-app calls within the same ACA environment only, excluding external reverse proxies. The AGW backend pool stayed Unhealthy(404) against the internal-ingress FQDN and every other AGW-fronted app in the same shared environment (all using `external_enabled = true`) was Healthy; switching this app to `external_enabled = true` matches the only pattern proven to work in this environment. The environment's own internal-only VNet configuration (no public inbound IP) remains the actual security boundary, not this app-level flag. | No longer blocking; superseded Task 6's original "internal HTTPS ingress (`external = false`)" checklist wording |
