...
From each portal web server, collect the following data points from the Application Gateway WAF access log and from the application HTTP response codes. The rate-limit middleware's `source_ip_not_allowed` event also records normalized `peerIp`, `peerTrusted`, `forwardedHeaderCount`, `forwardedHopCount`, `sourceResolution`, and (when available) `resolvedClientIp`; it never records the raw `X-Forwarded-For` header. The regular `AccessLog` event remains limited to `traceId`, `method`, `path`, `status`, and `durationMs`.
